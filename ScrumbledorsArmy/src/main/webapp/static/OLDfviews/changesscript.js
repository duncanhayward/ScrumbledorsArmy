...
//	 $scope.changeToScrum= function(){
		 ...
		 $scope.addBoard = function() {
	         ...
	         var board = {
	        		 id      	 : "25",
	        		 name 		 : "SCRUMbleBoard",
	        		 description : "This is a SCRUM Board for SCRUMbledore's Army"
	         };

	
	         $http({
	             method : 'POST',
	             url : 'http://localhost:8085/ScrumbledorsArmy/addBoard',        
	             data : board
	         }).then(function successCallback(response) {

	             $scope.response = JSON.stringify(response.data.board);
	             console.log('response.data->' + $scope.response);
	             console.log(response.status);
	             if (response.status != 200) {
	                 console.log('Insert Board Failed!!');

	             } else {
	                 console.log('Insert Board Successfull');
...
.controller('scrumCtrl', function($scope, $compile, $location, $http){
	...
	$scope.nameLane= function(){
		...
		$scope.laneName = document.getElementById('nameLane').value;
		
		console.log($scope.laneName);
		...
		$scope.addLane = function() {
	         ...
	         $http({
	             method : 'POST',
	             url : 'http://localhost:8085/ScrumbledorsArmy/addLane',        
	             data : lane
	         }).then(function successCallback(response) {
	             ...
	             console.log(response.status);
	             if (response.status != 200) {
	                 console.log('Insert Lane Failed!!');

	             } else {
	                 console.log('Insert Lane Successfull');
	                 $location.path('/scrumboard');
	                 // $('.login').css('display', 'none');
	             }
	...
	$scope.writeStory= function(){
		...
		var userStory = angular.element("<div class='modal fade in' id='addStoryModal' role='dialog' style='display: block;'><div class='modal-dialog'><div class='modal-content'><div class='modal-header'><h4 class='modal-title'>Add Story</h4><button type='button' class='close' ng-click='close($event)' data-dismiss='modal'>&times;</button></div><div class='modal-body' id='addStory'><textarea id='storyBox' placeholder='Enter Story Here' ng-keyup='addDates()'></textarea></div></div></div></div>");
...
//		if(event.keyCode == 13){
			...
			var expected = document.getElementById('end').value;
			var pointValue = document.getElementById('points').value;
		...
		$scope.storyDesc = document.getElementById('storyBox').value;
		
		$scope.addStory = function() {
			//ALTER WITH DATE AND DONE
	         var story = {
	        		 storyPost : {
	        		 id      	 : cardId,
	        		 points : pointValue,
	         		 description : $scope.storyDesc,
	         		 start: startDate,
	         		 endExpected: expected,
	         		 done: 'N'
	        		 },
	         ...
	         $http({
	             method : 'POST',
	             url : 'http://localhost:8085/ScrumbledorsArmy/addStory',        
	             data : story
	         }).then(function successCallback(response) {

	             $scope.response = JSON.stringify(response.data.story);
	             console.log('response.data->' + $scope.response);
	             console.log(response.status);
	             if (response.status != 200) {
	                 console.log('Insert Story Failed!!');

	             } else {
	                 console.log('Insert Story Successfull');
	                 $location.path('/scrumboard');
	                 // $('.login').css('display', 'none');
	             }
		...
		document.getElementById('start').parentNode.parentNode.parentNode.parentNode.parentNode.remove();
		}
//	}
	
	$scope.addDates=function(){
		var addedStory = document.getElementById('addStory');
		
		if(event.keyCode == 13){
		var dates = angular.element("<h2>Enter Start Date:</h2><input type='date' id='start' /><br><h2>Enter Expected End Date:</h2><input type='date' id='end'/><br><h2>Enter Point Value:</h2><input type='number' id='points' /><br></br><div class='btn btn-info' id='addCardBtn' ng-click='addCard()'>Add Story</div>")
		var theDates = $compile(dates)($scope);
			
		
		angular.element(addedStory).append(theDates);
		}
	}
...
//		document.getElementById('addTask').remove();
	...
	$scope.addTask=function(){
		...
		if(event.keyCode == 13){
			...
			$scope.persisTask = function() {
		         var task = {
		        		 taskPost : {
		        		 id      	 : taskId,
		         		 description : document.getElementById(taskId-1).value,
		         		 done: 'N'
		        		 },
		         ...
		         $http({
		             method : 'POST',
		             url : 'http://localhost:8085/ScrumbledorsArmy/addTask',        
		             data : task
		         }).then(function successCallback(response) {
		             ...
		             if (response.status != 200) {
		                 ...
		                 console.log('Insert Task Successfull');
		                 $location.path('/scrumboard');
		                 // $('.login').css('display', 'none');
		             }
...
//		var element1 = angular.element($event.target.parentNode.parentNode);
		...
		var views = angular.element("<div class='modal fade in' id='viewTaskModal' role='dialog' style='display: block;'><div class='modal-dialog'><div class='modal-content'><div class='modal-header'><h4 class='modal-title'>View Task(s)</h4><button type='button' class='close' ng-click='close($event)' data-dismiss='modal'>&times;</button></div><div class='modal-body'><ul id='tasks'></ul></div></div></div></div></div>");
...
//				for(j=0;j<Tasks[i].length;j++){
				var thetask = angular.element("<li>" + Tasks[i][taskId-1] + "</li>");
...
//				}
	...
	$scope.close=function($event){
		...
		var modalId =  $event.target.parentNode.parentNode.parentNode.parentNode.id;
